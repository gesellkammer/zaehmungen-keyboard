...
/*
	...
	You can change any value here to adjust to your setup
*/

;; MIDI Configuration
#define MIDICHANNEL       # 1 #   ; which channel to listen to (0 to listen to all)
#define CC_sustain        # 8 #  ; sustain pedal
#define CC_volume         # 1 #   ; expression pedal used to control gain

#define volumePedalCurve  # 2.5 # ; coefficient for gain pedal curve (1=linear)
...
#define SCOPE_PERIOD #0.01#
#define SCOPE_NUMPERIODS #1#
...
opcode setSustainPedal, 0, k
	...
	if kstate == 1 kgoto finish
	
	; ftprint gi_keybNotesSustained, 
	...
	ftset gi_keybNotesSustained, k(0)
finish:
endop
...
instr setup
	...
	outvalue "graph1", "@find audio aAudioPreCompressor"
	outvalue "graph2", "@find audio aAudioPostLimiter"
...
instr io
	...
	gk_noteonMaxDb chnget "noteonMaxDb"
	gk_ntoeonMinDb chnget "noteonMinDb"
	gk_rnd         chnget "randomness"
...
instr midiIn
	...
	if kstatus == 0 goto finish
	if kchan != $MIDICHANNEL then
...
instr noteon		; (midinote, velocity)
	...
	if table:i(imidinote, gi_keybNotesHeld) == 1 then
...
instr noteoff
	...
	if gk_keybSustainPedal == 1 then
		tabw(k(1), imidinote, gi_keybNotesSustained)
		kgoto finish
	endif
...
instr master
	...
	kcompGainCompDb chnget "compGainComp"
	kmasterGainDb chnget "masterGain"
	...
	aAudioPreCompressor = gaL
	aAudioPostLimiter = aL
	
	display aAudioPreCompressor, $SCOPE_PERIOD, $SCOPE_NUMPERIODS
	display aAudioPostLimiter, $SCOPE_PERIOD, $SCOPE_NUMPERIODS
